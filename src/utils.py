    ...
    if len(resp["centers"]) >= 0:
        for center in resp["centers"]:
            for session in center["sessions"]:
                # Cowin uses slot number for display post login,
                # but checks available_capacity before booking appointment is allowed
...
def collect_user_details(request_header):
    ...
    Or Smart search State/District for selected Pincodes ? \n \
    Enter 1 for Pincode or 2 for State/District or 3 for State/District filter by Pincodes \
    (Optimized for rate-limit) (Default 2): """)
